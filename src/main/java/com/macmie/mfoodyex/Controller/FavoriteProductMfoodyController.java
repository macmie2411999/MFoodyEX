...
public class FavoriteProductMfoodyController {
    ...
    public ResponseEntity<?> getAllFavoriteProductMfoodys(Principal principal) {
        ...
        if (FavoriteProductMfoodyList.isEmpty()) {
            return new ResponseEntity<>("NO_CONTENT List of FavoriteProductMfoodys", HttpStatus.NO_CONTENT);
        }
    ...
    public ResponseEntity<?> getAllFavoriteProductMfoodysByIdFavoriteListProducts(@PathVariable("ID") int ID,
            Principal principal) {
        log.info("Get List of FavoriteProductMfoodys with idFavoriteListProducts: {} by {}", ID, principal.getName());
        ...
        if (!applicationCheckAuthorController.checkAuthorization(principal,
                FavoriteListProductsMfoodyInterfaceService.getFavoriteListProductsMfoodyByID(ID).getUser()
                        .getIdUser())) {
        ...
        if (FavoriteProductMfoodyList.isEmpty()) {
            return new ResponseEntity<>("NO_CONTENT List of FavoriteProductMfoodys with idFavoriteListProducts: " + ID,
    ...
    public ResponseEntity<?> getAllFavoriteProductMfoodysByIdProduct(@PathVariable("ID") int ID, Principal principal) {
        ...
        if (FavoriteProductMfoodyList.isEmpty()) {
            return new ResponseEntity<>(
                    "NO_CONTENT List of FavoriteProductMfoodys with idProduct: " + ID, HttpStatus.NO_CONTENT);
        }
        return new ResponseEntity<>(FavoriteProductMfoodyList, HttpStatus.OK);
    }
    ...
    public ResponseEntity<?> getFavoriteProductMfoodyByID(
        ...
        if (currentFavoriteListProductsMfoody == null) {
            return new ResponseEntity<>("NOT_FOUND FavoriteListProductsMfoody with ID: " + idFavoriteListProducts,
        ...
        // FavoriteProductMfoody
        if (!applicationCheckAuthorController.checkAuthorization(principal,
                currentFavoriteListProductsMfoody.getUser().getIdUser())) {
            return new ResponseEntity<>("FORBIDDEN Authorization failed!", HttpStatus.FORBIDDEN);
        }

        FavoriteProductMfoody FavoriteProductMfoody = FavoriteProductMfoodyInterfaceService
                .getFavoriteProductMfoodyByIFavoriteListProductsAndIdProduct(idFavoriteListProducts, idProduct);
        if (FavoriteProductMfoody == null) {
            return new ResponseEntity<>(
                    "NOT_FOUND FavoriteProductMfoody with idFavoriteListProducts: " + idFavoriteListProducts
                            + ", idProduct: " + idProduct,
                    HttpStatus.NOT_FOUND);
        }
    ...
    public ResponseEntity<?> deleteFavoriteProductMfoodyByID(
        ...
        try {
            FavoriteProductMfoodyInterfaceService
                    .deleteFavoriteProductMfoodyByIdFavoriteProductMfoody(idFavoriteListProducts, idProduct);
        } catch (Exception e) {
            log.error(
    ...
    public ResponseEntity<?> deleteAllFavoriteProductMfoodysByIdFavoriteListProducts(@PathVariable("ID") int ID,
        ...
        try {
            FavoriteProductMfoodyInterfaceService.deleteAllFavoriteProductsMfoodyByIdFavoriteListProducts(ID);
        } catch (Exception e) {
            log.error("An error occurred while deleting List of FavoriteProductMfoodys with idFavoriteListProducts: "
                    + ID);
            log.error("Detail Error: " + e);
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR,
                    "INTERNAL_SERVER_ERROR Exceptions occur when deleting List of FavoriteProductMfoodys");
        }
    ...
    public ResponseEntity<?> deleteAllFavoriteProductMfoodysByIdProduct(@PathVariable("ID") int ID,
        ...
        try {
            ...
            log.error("An error occurred while deleting List of FavoriteProductMfoodys with idProduct: " + ID);
            log.error("Detail Error: " + e);
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR,
                    "INTERNAL_SERVER_ERROR Exceptions occur when deleting List of FavoriteProductMfoodys");
        }
    ...
    public ResponseEntity<?> editFavoriteProductMfoody(@RequestBody String FavoriteProductPOJOJsonObject,
        ...
        try {
            ...
            Gson gson = new Gson();
            FavoriteProductMfoodyPOJO newFavoriteProductMfoodyPOJO = gson.fromJson(FavoriteProductPOJOJsonObject,
            ...
            if (newProductMfoody == null) {
                return new ResponseEntity<>(
                        "NOT_FOUND ProductMfoody with ID: " + newFavoriteProductMfoodyPOJO.getIdProduct(),
                        HttpStatus.NOT_FOUND);
            }
            ...
            if (newFavoriteProductMfoody == null) {
                return new ResponseEntity<>(
                        "NOT_FOUND FavoriteProductMfoody with idFavoriteListProducts: "
                                + newFavoriteProductMfoodyPOJO.getIdFavoriteListProducts()
                                + " and idProduct: " + newFavoriteProductMfoodyPOJO.getIdProduct(),
                        HttpStatus.NOT_FOUND);
            }
    ...
    public ResponseEntity<?> addNewFavoriteProductMfoody(@RequestBody String FavoriteProductPOJOJsonObject,
        ...
        try {
            ...
            FavoriteProductMfoodyPOJO newFavoriteProductMfoodyPOJO = gson.fromJson(FavoriteProductPOJOJsonObject,
                    FavoriteProductMfoodyPOJO.class);
            FavoriteProductMfoody newFavoriteProductMfoody = newFavoriteProductMfoodyPOJO.renderFavoriteProductMfoody();
            ...
            if (FavoriteListProductsMfoodyInterfaceService.getFavoriteListProductsMfoodyByID(
                ...
                return new ResponseEntity<>("NOT_FOUND FavoriteListProductsMfoody with ID: "
                        + newFavoriteProductMfoodyPOJO.getIdFavoriteListProducts(), HttpStatus.NOT_FOUND);
            }
            ...
            newFavoriteProductMfoody.setIdFavoriteProductMfoody(
                    new FavoriteProductMfoodyId(newFavoriteProductMfoodyPOJO.getIdFavoriteListProducts(),
                            newFavoriteProductMfoodyPOJO.getIdProduct()));
            ...
            newFavoriteProductMfoody.setFavoriteListProducts(FavoriteListProductsMfoodyInterfaceService
                    .getFavoriteListProductsMfoodyByID(newFavoriteProductMfoodyPOJO.getIdFavoriteListProducts()));
        ...
        try {
            ...
            for (FavoriteProductMfoodyPOJO product : productList) {
                ...
                FavoriteProductMfoody existingProductFavoriteListProducts = FavoriteProductMfoodyInterfaceService
                        .getFavoriteProductMfoodyByIFavoriteListProductsAndIdProduct(
                                product.getIdFavoriteListProducts(), product.getIdProduct());

                if (existingProductFavoriteListProducts == null) {
                    // If the product is not in the FavoriteListProducts, add a new product
                    ResponseEntity<?> addResult = addNewFavoriteProductMfoody(gson.toJson(product), principal);
                    ...
                    ResponseEntity<?> updateResult = editFavoriteProductMfoody(gson.toJson(product), principal);
