...
public class CreditCardMfoodyController {
    ...
    public ResponseEntity<?> getAllCreditCardMfoodys(Principal principal) {
        ...
        //     return new ResponseEntity<>("NOT_FOUND List of CreditCards", HttpStatus.NOT_FOUND);
        // }
    ...
    public ResponseEntity<?> getAllCreditCardMfoodysByIdUser(@PathVariable("ID") int ID, Principal principal) {
        ...
        List<CreditCardMfoody> creditCardMfoodyList = creditCardMfoodyInterfaceService
                .getListCreditCardMfoodysByIdUser(ID);
        // if (creditCardMfoodyList.isEmpty()) {
        //     return new ResponseEntity<>("NO_CONTENT List CreditCards of UserMfoody with idUser: " + ID,
    ...
    public ResponseEntity<?> editCreditCardMfoody(@RequestBody String creditCardPOJOJsonObject, Principal principal) {
        try {
            // Convert JsonObject to CreditCardPOJO object, Check input idCard and attach
            // UserMfoody to CreditCardMfoody
            Gson gson = new Gson();
            CreditCardMfoodyPOJO newCreditCardPOJO = gson.fromJson(creditCardPOJOJsonObject,
                    CreditCardMfoodyPOJO.class);
            ...
            UserMfoody attachUserMfoody = userMfoodyInterfaceService
                    .getUserMfoodyByIdCard(newCreditCardPOJO.getIdCard());
            ...
            log.info("CreditCardMfoody with ID: {} by {} is edited", newCreditCardMfoody.getIdCard(),
                    principal.getName());
    ...
    /*
     * 1. The threat is any UserMfoodys can create CreditCardMfoody using different
     * idUser
    ...
    public ResponseEntity<?> addNewCreditCardMfoody(@RequestBody String creditCardPOJOJsonObject, Principal principal) {
        try {
            ...
            // userMfoodyInterfaceService.getUserMfoodyByID(newCreditCardPOJO.getIdUser());
            ...
            // Save CreditCard to DB and return (Updated Cart in DB could have ID differs
            // from user's request)
